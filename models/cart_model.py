...
def obtener_o_crear_carrito(usuario_id):
    """Busca el ID del carrito del usuario. Si no existe, lo crea."""
...
def agregar_producto_al_carrito(usuario_id, producto_id, cantidad=1):
    ...
    if detalle:
        ...
        # Si no existe, se inserta la nueva fila
...
def obtener_detalles_carrito(usuario_id):
    """Devuelve todos los productos dentro del carrito de un usuario con sus totales."""
    carrito_id = obtener_o_crear_carrito(usuario_id)
    conexion = obtener_conexion()
    cursor = conexion.cursor(dictionary=True)
...
def eliminar_del_carrito(detalle_id):
    """Elimina una fila específica del detalle del carrito."""
    conexion = obtener_conexion()
